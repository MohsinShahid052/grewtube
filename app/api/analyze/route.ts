...
export async function POST(req: NextRequest) {
  try {
    ...
    const { positiveCommentCount, negativeCommentCount, voteCounts } = await analyzeCommentsSentimentAndVotes(loadedModel, videoId);
...
async function analyzeCommentsSentimentAndVotes(model: tf.LayersModel, videoId: string) {
  try {
    ...
    for (const item of data.items) {
      ...
      // Count occurrences of A1, B2, C*3
      count_A1 += (comment.match(/\bA1\b/gi) || []).length;
      count_B2 += (comment.match(/\bB2\b/gi) || []).length;
      count_C3 += (comment.match(/\bC\*3\b/gi) || []).length;      
    }
